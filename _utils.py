...
def evaluate_dataset(dataset, model, flavor=None, compute_ari=False, ari_ks=np.arange(3,16,1)):
    dev = next(model.parameters()).device
    data = dataset.data.to(dev)
    data_lib = dataset.data_max.to(dev)

    if flavor == "embryotime":
        labels = dataset.time_raw_unmasked.to(dev)
        mask = (dataset.time_raw.to(dev) != -1)
    elif flavor == "cardiotoxin":
        labels = dataset.compound_unmasked.to(dev)
        mask = (dataset.compound.to(dev) != -1)
    ...
    with torch.no_grad():
        ...
        label_preds = x_hat[:,-1]
        metrics["rmse"] = F.mse_loss(x_hat[:,:-1]*data_lib, data[:,:-1], reduction='mean').item()
        metrics["mae"] = F.l1_loss(x_hat[:,:-1]*data_lib, data[:,:-1], reduction='mean').item()
        ...
        if flavor == "embryotime":
            ...
            label_preds = F.sigmoid(label_preds)
        ...
        if compute_ari:
            subgroup = "compound" if flavor == "cardiotoxin" else "cell_type"
            res, k, res = best_ari(mu, dataset.obs[subgroup], ari_ks)
