...
- C. elegans embryogenesis [1], [GEO: GSE126954][celegans]. Files to be placed in `data/celegans/` with: cell annotations as `annotations.csv`, transcript names as `genes.csv` and counts as `counts.mtx`.
- Irf8-cKO mouse brains [2], [GEO: GSE128855][irf8]. Files to be placed in `data/mousebrain/` with: cell annotations as `barcodes.tsv`, transcript names as `genes.tsv` and counts as `counts.mtx`.
- Cardiotoxin-induced injury in mice [3], [ArrayExpress: E-MTAB-9715][ctx]. Files to be placed in `data/cardiotoxin/` with: cell annotations as `cells.mtx_cols`, transcript names as `features.mtx_rows` and counts as `counts.mtx`.

2. For each dataset, run the relevant preprocessing script with prefix `'pre_'`. 

3. Finally, the `.ipynb` notebooks can be run to generate the results. 

**Note**: Ensure you have all the necessary libraries and dependencies installed before running the files. This readme will be extended upon approval.
...
[ctx]: https://www.ebi.ac.uk/biostudies/arrayexpress/studies/E-MTAB-9715
